...
def main() -> None:
    ...
    ap.add_argument("--out", default="outputs/ntu120_embeddings.npz")
    ap.add_argument("--metadata-out", default="outputs/ntu120_embeddings_metadata.csv")
    ...
    model = SimpleSkeletonEncoder().to(device)
    state = torch.load(args.checkpoint, map_location=device)
    model.load_state_dict(state)
    model.eval()

    # Keep only experiment-relevant actions/roles; excluded classes need not be embedded.
    ...
    for i, (x, meta) in enumerate(loader, start=1):
        z = model(x.to(device)).cpu().numpy().astype(np.float32, copy=False)
        embeddings.append(z)
