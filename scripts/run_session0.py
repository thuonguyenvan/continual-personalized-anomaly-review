...
def main():
    ...
    parser.add_argument("--checkpoint", required=True, help="Trained encoder checkpoint")
    ...
    detector_calib = NTUManifestDataset(
        args.manifest, args.root, inner_split="detector_calib",
        roles=["global_normal"], seq_len=args.seq_len,
    )
    retention = NTUManifestDataset(
        args.manifest, args.root, inner_split="retention_val",
        roles=["global_normal"], seq_len=args.seq_len,
    )
    deployment = NTUManifestDataset(
        args.manifest, args.root, inner_split="deployment_test",
    ...
    if any(x.numel() == 0 for x in [z_train, z_calib, z_retention, z_test]):
        raise RuntimeError("One or more required partitions are empty. Regenerate the manifest with the current script.")

    detector = PrototypeDetector()
    detector.fit(z_train.numpy(), threshold_quantile=args.threshold_quantile)
    threshold = threshold_from_quantile(detector.score_samples(z_calib.numpy()), q=args.threshold_quantile)

    retention_scores = detector.score_samples(z_retention.numpy())
    test_scores = detector.score_samples(z_test.numpy())
    ...
    retention_fpr = float((retention_scores > threshold).mean())
    print(f"retention_global_fpr: {retention_fpr:.6f}")
    if deploy_global_scores:
        deployment_global_fpr = sum(s > threshold for s in deploy_global_scores) / len(deploy_global_scores)
        print(f"deployment_global_fpr: {deployment_global_fpr:.6f}")
