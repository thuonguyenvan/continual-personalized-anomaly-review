...
class ActionClassifier(nn.Module):
    def __init__(self, embed_dim: int = 128, num_classes: int = len(GLOBAL_NORMAL_ACTIONS)):
        super().__init__()
        self.encoder = STGCNEncoder(embed_dim=embed_dim)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train ST-GCN-style NTU120 encoder on global-normal actions only")
    ap.add_argument("--manifest", required=True)
    ap.add_argument("--root", required=True)
    ap.add_argument("--out", default="outputs/ntu120_pilot_v0.1/checkpoints/stgcn_encoder.pt")
    ...
    with history_out.open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["epoch", "lr", "train_loss", "train_acc", "val_loss", "val_acc"])
        w.writeheader()
        w.writerows(history)
